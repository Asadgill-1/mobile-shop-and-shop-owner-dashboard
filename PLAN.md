...
- ✅ **Gaps wave** (2026-07-21, backend migration 023): five owner-requested gaps.
  - **Date-based refs** — `ODR-DD-MM-NNN` / `INV-DD-MM-NNN` (day-month-daily seq per shop). Display layer only: the bigint keys stay; `lib/types.ts::orderRef`/`invoiceRef` render from `created_at`+`day_seq` (allocated by the backend `next_day_seq` RPC at insert). Pre-023 rows fall back to `#{n}` / `INV-000042`.
  - **Home delivery charge** — a delivery-fee field on the confirm control (`components/order-actions.tsx`); the customer gets a breakdown; invoice adds a delivery line; reports show delivery income (collected vs rider-kept). A `rider_keeps_delivery` per-shop toggle in Settings drives the COD split (backend `deliver_order`).
