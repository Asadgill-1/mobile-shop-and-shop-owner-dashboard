...
import { setRiderKeepsDelivery } from "@/actions/settings";
import type { ActionResult } from "@/actions/orders";
import { Feedback } from "./action-feedback";

/** On = riders keep the delivery fee; off = they hand all cash to the shop (023). */
export function RiderDeliveryToggle({ shopId, enabled }: { shopId: string; enabled: boolean }) {
  const [result, setResult] = useState<ActionResult | null>(null);
  const [pending, startTransition] = useTransition();

  const flip = () =>
    startTransition(async () => setResult(await setRiderKeepsDelivery(shopId, !enabled)));
  ...
  return (
    <div className="flex flex-col gap-2">
      <button
        ...
        aria-label="Riders keep the delivery fee"
        onClick={flip}
        disabled={pending}
